...
export function getRideStatusDisplay(status: RideStatus): { label: string; kind: StatusPulseKind } {
  if (status === 'Completed') return { label: 'Завершено', kind: 'online' };
  if (status === 'Canceled') return { label: 'Скасовано', kind: 'offline' };
  if (status === 'InRide') return { label: 'У дорозі', kind: 'inRide' };
  if (status === 'Accepted') return { label: 'Прийнято', kind: 'accepted' };
  return { label: 'Створено', kind: 'created' };
}
